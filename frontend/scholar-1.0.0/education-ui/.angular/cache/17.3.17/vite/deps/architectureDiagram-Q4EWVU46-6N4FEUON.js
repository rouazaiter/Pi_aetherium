  ...
  getDataStructures() {
    if (this.dataStructures === void 0) {
      ...
      const notVisited = Object.keys(adjList).reduce(
        (prev, id) => id === firstId ? prev : __spreadProps(__spreadValues({}, prev), { [id]: 1 }),
        {}
      );
...
var populateDb = __name((ast, db) => {
  ...
  ast.services.map((service) => db.addService(__spreadProps(__spreadValues({}, service), { type: "service" })));
  ast.junctions.map((service) => db.addJunction(__spreadProps(__spreadValues({}, service), { type: "junction" })));
...
var drawEdges = __name(function(edgesEl, cy, db, diagramId) {
  return __async(this, null, function* () {
    ...
    yield Promise.all(
      cy.edges().map((edge) => __async(this, null, function* () {
        const {
          source,
          sourceDir,
          sourceArrow,
...
var drawGroups = __name(function(groupsEl, cy, db, diagramId) {
  return __async(this, null, function* () {
    const padding = db.getConfigField("padding");
    ...
    const halfIconSize = iconSize / 2;
    yield Promise.all(
      cy.nodes().map((node) => __async(this, null, function* () {
        ...
        if (data.type === "group") {
          ...
          if (data.icon) {
            ...
            bkgElem.html(
              `<g>${yield getIconSVG(data.icon, { height: groupIconSize, width: groupIconSize, fallbackPrefix: architectureIcons.prefix })}</g>`
            );
            bkgElem.attr(
              "transform",
...
var drawServices = __name(function(db, elem, services, diagramId) {
  return __async(this, null, function* () {
    ...
    for (const service of services) {
      ...
      if (service.icon) {
        bkgElem.html(
          `<g>${yield getIconSVG(service.icon, { height: iconSize, width: iconSize, fallbackPrefix: architectureIcons.prefix })}</g>`
        );
      } else if (service.iconText) {
        bkgElem.html(
          `<g>${yield getIconSVG("blank", { height: iconSize, width: iconSize, fallbackPrefix: architectureIcons.prefix })}</g>`
        );
...
var draw = __name((text, id, _version, diagObj) => __async(void 0, null, function* () {
  const db = diagObj.db;
  db.setDiagramId(id);
...
//# sourceMappingURL=architectureDiagram-Q4EWVU46-6N4FEUON.js.map

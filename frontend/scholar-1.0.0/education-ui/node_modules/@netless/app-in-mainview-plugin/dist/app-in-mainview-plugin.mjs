...
var w = (l, e, t) => e in l ? f(l, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : l[e] = t;
var n = (l, e, t) => w(l, typeof e != "symbol" ? e + "" : e, t);
import { toJS as v, autorun as M, InvisiblePlugin as A, isRoom as S } from "white-web-sdk";
import y from "eventemitter2";
import { isEqual as x, isBoolean as m } from "lodash";
class C {
  constructor(e) {
    n(this, "_map");
...
class V {
  ...
  getAttributes() {
    return v(this.plugin.attributes) || {};
  }
  diff(e) {
    const t = P(e);
    for (const i of t)
      this.storageObserver.has(i) ? x(this.storageObserver.get(i), e[i]) || this.storageObserver.set(i, e[i]) : this.storageObserver.set(i, e[i]);
    for (const i of [...this.storageObserver.keys()])
      t.includes(i) || this.storageObserver.delete(i);
  }
  observeStorage() {
    this.stateDisposer = M(async () => {
...
const g = "0.0.9", H = "@netless/app-in-mainview-plugin";
if (typeof window < "u") {
...
const d = {
  ...
  language: "en",
  theme: "light"
}, b = "default-app-in-main-view-plugin", T = {
  en: {
    show: "show all",
    hidden: "hidden all"
  },
...
class E {
  constructor(e) {
    ...
    n(this, "i18n");
    n(this, "theme");
    n(this, "isBindContainer", !1);
    n(this, "containerClickHandler", (e) => {
      if (e.stopPropagation(), e.stopImmediatePropagation(), this.manager.control.wm.readonly)
        return;
      getComputedStyle(this.menuView).display === "flex" ? this.menuView.style.display = "none" : this.menuView.style.display = "flex";
    });
    n(this, "menuViewClickHandler", (e) => {
      e.stopPropagation(), e.stopImmediatePropagation();
      const t = e.target, i = t.getAttribute(`data-${this.c("app-id")}`);
      if (i) {
    ...
    n(this, "onFullscreenChangeHandler", (e) => {
      e ? this.container.style.display = "none" : this.container.style.display = "block";
    });
  ...
  bindContainer() {
    ...
    e && (e.insertAdjacentElement("afterend", this.container), this.isBindContainer = !0);
  }
  removeContainer() {
    const e = this.container.parentElement;
    e && (e.removeChild(this.container), this.isBindContainer = !1);
  }
  ...
  async init() {
    const e = await this.manager.getApps();
...
class B {
  constructor(e) {
    ...
    n(this, "timer");
    n(this, "control");
    this.control = e.control, this.enableDefaultUI = m(e.options.enableDefaultUI) ? e.options.enableDefaultUI : d.enableDefaultUI, this.onlyShowHidden = m(e.options.onlyShowHidden) ? e.options.onlyShowHidden : d.onlyShowHidden, this.scenePath = this.currentScenePath, this.apps = this.getCurrentApps(), this.enableDefaultUI && (this.appMenu = new E({
      manager: this,
  ...
  async getApps() {
    return this.checkAppChangeAllReady() ? this.apps : (await this.willAppInfoAllReady(), this.apps);
  }
  updateAppInfo(e) {
    const t = this.apps.get(e);
    t && (t.appInfo = this.wmAppProxies.get(e), this.resolvePublicEventEmitter && this.checkAppChangeAllReady() && this.resolvePublicEventEmitter(!0));
  }
  async pageChangeHandler(e, t) {
    this.scenePath = e, this.apps = this.getCurrentApps(t), await this.willPublicEmitterMenuChange();
  }
  ...
  async willAppInfoAllReady(e) {
    if (this.resolvePublicEventEmitter && this.resolvePublicEventEmitter(!1), this.checkAppChangeAllReady()) {
      e && e();
      return;
    }
    const t = await new Promise((i) => {
      this.resolvePublicEventEmitter = i, this.resolveTimer = setTimeout(() => {
        this.resolveTimer = void 0, this.resolvePublicEventEmitter && this.resolvePublicEventEmitter(!0);
      }, 2e3);
    });
    this.resolveTimer && (clearTimeout(this.resolveTimer), this.resolveTimer = void 0), this.resolvePublicEventEmitter = void 0, t && e && e();
  }
  async willPublicEmitterMenuChange() {
...
class z {
  constructor(e) {
    ...
    n(this, "onWritableChange", () => {
      const e = this.wm.displayer.isWritable;
      this.isCurWritable = e;
    });
    n(this, "observeTitlebarHandler", () => {
      const e = this.wm.boxState;
      let t = !1;
      const i = this.getCurrentPageVisibleApps();
      e === "maximized" ? (i.size < 2 ? this.setTitlebarNodeDisplay("none") : this.setTitlebarNodeDisplay("flex"), t = !0) : this.setTitlebarNodeDisplay("none");
      for (const r of i)
        t ? this.setAppNodeDisplay(r, "block", !0) : this.activeMaximizeBtn(r, !1);
    });
    ...
    n(this, "getTargetParent", (e) => e.hasAttribute("data-tele-box-i-d") ? e : e.parentElement ? this.getTargetParent(e.parentElement) : null);
    n(this, "minimizeBtnClickHandler", (e) => {
      if (e.stopPropagation(), e.stopImmediatePropagation(), this.wm.readonly)
        return;
      const t = this.getTargetParent(e.target);
      if (t) {
        const i = t.getAttribute("data-tele-box-i-d");
        i && this.hideApp(i);
      }
    });
    n(this, "observeAppSetupHandler", (e) => {
      if (document.querySelector(`div.telebox-box[data-tele-box-i-d="${e}"]`)) {
        const i = this.collector.getAppState(e), r = this.wm.mainView.focusScenePath;
        i ? (i.status === "visible" && r === i.scenePath ? this.setAppNodeDisplay(e, "block") : i.status === "hidden" && this.setAppNodeDisplay(e, "none"), this.wm.boxState === "maximized" && (this.observeTitlebarTimer(), this.observerFocusAppTimer())) : r && this.collector.addAppState(e, {
          scenePath: r,
          status: "visible"
        });
        const a = document.querySelector(`div[data-tele-box-i-d="${e}"] .telebox-titlebar-icon-minimize`);
        a && (a.removeEventListener("click", this.minimizeBtnClickHandler), a.addEventListener("click", this.minimizeBtnClickHandler));
      }
      this.appMenuManager.updateAppInfo(e);
    });
    n(this, "observeBoxCloseHandler", (e) => {
      this.collector.storage.has(e.appId) && this.collector.deleteAppState(e.appId);
      const i = document.querySelector(`div[data-tele-box-i-d="${e.appId}"] .telebox-titlebar-icon-minimize`);
      i && i.removeEventListener("click", this.minimizeBtnClickHandler);
    });
    n(this, "observeMainViewScenePathChangeHandler", (e) => {
      this.collector.storage.forEach((i, r) => {
        i.scenePath === e && i.status === "visible" ? this.setAppNodeDisplay(r, "block") : this.setAppNodeDisplay(r, "none");
      }), this.wm.boxState === "maximized" && (this.observeTitlebarTimer(), this.observerFocusAppTimer());
      const t = this.getTargetPageApps(e);
      this.appMenuManager.pageChangeHandler(e, t);
    });
    n(this, "observeMaxStateMinimizeBtnClickHandler", (e) => {
      e.stopPropagation(), e.stopImmediatePropagation();
  ...
  setAppNodeDisplay(e, t, i = !1) {
    const r = Array.from(document.querySelectorAll(`[data-tele-box-i-d="${e}"]`));
    r.length && r.forEach((a) => {
      t === "block" ? (a.style.display = `${t}`, i && a instanceof HTMLDivElement && (this.wm.boxState === "maximized" ? this.activeMaximizeBtn(a, !0) : this.activeMaximizeBtn(a, !1))) : a.style.removeProperty("display");
    });
  }
  ...
  hideApp(e) {
    ...
    t && t.status === "visible" && this.collector.updateAppState(e, {
      ...t,
      status: "hidden"
    });
  }
  showApp(e) {
    const t = this.collector.getAppState(e);
    t && t.status === "hidden" && this.collector.updateAppState(e, {
      ...t,
      status: "visible"
    });
  }
...
const s = class s extends A {
  static async getInstance(e, t, i) {
    i && (s.logger = i);
    const r = e.displayer;
    let a = r.getInvisiblePlugin(s.kind);
    s.currentManager || s.createCurrentManager(e, t || {}), a || (a = await s.createAppInMainViewPlugin(r, s.kind)), a && s.currentManager && s.currentManager.bindPlugin(a);
    const c = {
      displayer: r,
      windowManager: e,
      currentManager: s.currentManager,
      destroy() {
        s.currentManager && (s.logger.info("[AppInMainViewPlugin] has been destroyed"), s.currentManager.destroy(), s.currentManager = void 0);
      },
      addListener: (o, h) => {
        var p;
        (p = s.currentManager) == null || p.publicEventEmitter.on(o, h);
      },
      removeListener: (o, h) => {
        var p;
        (p = s.currentManager) == null || p.publicEventEmitter.off(o, h);
      },
      hideApp: (o) => {
        var h;
        s.logger.info(`[AppInMainViewPlugin] hideApp ${o}`), (h = s.currentManager) == null || h.hideApp(o);
      },
      showApp: (o) => {
        var h;
        s.logger.info(`[AppInMainViewPlugin] showApp ${o}`), (h = s.currentManager) == null || h.showApp(o);
      },
      showCurrentPageApps: () => {
        var o;
        s.logger.info("[AppInMainViewPlugin] showCurrentPageApps"), (o = s.currentManager) == null || o.showCurrentPageApps();
      },
      hiddenCurrentPageApps: () => {
        var o;
        s.logger.info("[AppInMainViewPlugin] hiddenCurrentPageApps"), (o = s.currentManager) == null || o.hiddenCurrentPageApps();
      }
    };
    return Object.defineProperty(c, "currentPageVisibleApps", {
      get() {
        return s.currentManager ? s.currentManager.getCurrentPageVisibleApps() : /* @__PURE__ */ new Set();
      }
    }), Object.defineProperty(c, "currentPageApps", {
      get() {
        return s.currentManager ? s.currentManager.getCurrentPageApps() : /* @__PURE__ */ new Map();
      }
    }), e._appInMainViewPlugin = c, e._appInMainViewPlugin;
  }
  static onCreate(e) {
    e && s.currentManager && (s.timer && (clearTimeout(s.timer), s.timer = void 0), s.currentManager.bindPlugin(e));
  }
  static async createAppInMainViewPlugin(e, t) {
    if (S(e))
      try {
        if (e.isWritable)
          return await e.createInvisiblePlugin(s, {});
        {
          await e.setWritable(!0);
          const r = await s.createAppInMainViewPlugin(e, t);
          return await e.setWritable(!1), r;
        }
      } catch (r) {
        s.logger.error("[AppInMainViewPlugin] createAppInMainViewPlugin error", r);
      }
    let i = e.getInvisiblePlugin(t);
    return i || (await new Promise((r) => {
      s.timer && (clearTimeout(s.timer), s.timer = void 0), s.timer = setTimeout(() => {
        s.timer = void 0, r(!0);
      }, 1e3);
    }), i = await s.createAppInMainViewPlugin(e, t)), i;
  }
  destroy() {
    var e;
    s.logger.info("[AppInMainViewPlugin] passive destroyed"), (e = s.currentManager) == null || e.destroy(), s.currentManager = void 0;
  }
};
...
//# sourceMappingURL=app-in-mainview-plugin.mjs.map

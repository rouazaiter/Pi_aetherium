...
declare let useFreezer: boolean;
...
declare function syncSceneWithSlide(room: Room, context: AppContext<Attributes>, slide: Slide, baseScenePath: string): void;
...
type MagixEventListener = Parameters<AppContext<Attributes, MagixEvents>["addMagixEventListener"]>[1];
declare class SlideControllerBase {
    ...
    protected readonly sideEffect: SideEffectManager;
    ...
    previewList: string[];
    constructor(props: SlideControllerOptions);
    ready: boolean;
    protected resolveReady: (index: number) => void;
    readonly readyPromise: Promise<void>;
    jumpToPage(page: number, origin?: string): void;
    protected initialize(): void;
    protected kickStart(): void;
    protected registerEventListeners(): void;
    protected onSyncDispatch: (event: SyncEvent) => void;
    protected magixEventListener: MagixEventListener;
    protected syncStateOnce(): void;
    protected onStateChange: (state: SlideState) => void;
    protected pollCount: number;
    protected pollReadyState: () => void;
    protected _pageCount: number;
    get pageCount(): number;
    get page(): number;
    protected createSlide(anchor: HTMLDivElement, defaults?: Partial<ISlideConfig>): Slide;
...
declare class ResizableContainer {
    private root;
    ...
    setSlideObject(slide: Slide): void;
    private updateSlideSize;
    updateResizableContainer(): void;
    handleNormalizeTranslate(x: number, y: number, options: {
        triggerScrollBar: boolean;
        triggerSync: boolean;
    }): void;
    scaleContainer(applyScale: number): void;
...
declare class SlideDocsViewer {
    ...
    onPageChanged: () => void;
    protected _onPageChangedTimer: number;
    ...
    protected wrapClassName(className: string): string;
    protected namespace: string;
    protected getWhiteSnapshot(pageIndex: number, canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, slideWidth: number, slideHeight: number): Promise<void>;
    protected reportProgress(progress: number, result: {
        pdf: ArrayBuffer;
        title: string;
    } | null): void;
    protected toPdf: () => Promise<null | undefined>;
}
...
declare function previewSlide({ container, taskId, resourceList, url, debug, }: PreviewParams): SlidePreviewer;
declare class SlidePreviewer {
    ...
    protected hotkeyListener: (ev: KeyboardEvent) => void;

...
var FastboardAppBase = class {
  constructor(sdk, room, manager, hotKeys, syncedStore, appliancePlugin, appInMainViewPlugin) {
    this.sdk = sdk;
    this.room = room;
    this.manager = manager;
    this.hotKeys = hotKeys;
    this.syncedStore = syncedStore;
    this.appliancePlugin = appliancePlugin;
    this.appInMainViewPlugin = appInMainViewPlugin;
  ...
  /** @internal */
  _addApplianceListener(name, listener) {
    if (this._destroyed || !this.appliancePlugin)
      return noop2;
    this.appliancePlugin.addListener(name, listener);
    return () => {
      var _a;
      return (_a = this.appliancePlugin) == null ? void 0 : _a.removeListener(name, listener);
    };
  }
  /** @internal */
  _addAppInMainViewListener(name, listener) {
    if (this._destroyed || !this.appInMainViewPlugin)
      return noop2;
    this.appInMainViewPlugin.addListener(name, listener);
    return () => {
      var _a;
      return (_a = this.appInMainViewPlugin) == null ? void 0 : _a.removeListener(name, listener);
    };
  }
  /**
  ...
  async destroy() {
    ...
    (_a = this.appliancePlugin) == null ? void 0 : _a.destroy();
    (_b = this.appInMainViewPlugin) == null ? void 0 : _b.destroy();
...
var FastboardApp = class extends FastboardAppBase {
  constructor() {
    ...
    /** @internal */
    __publicField(this, "_visibleApps", /* @__PURE__ */ new Set());
    /**
     * visible Apps, when appInMainViewPlugin is not enabled, it will be empty.
     */
    __publicField(this, "visibleApps", readable(this._visibleApps, (set) => {
      if (!this.appInMainViewPlugin) {
        set(this._visibleApps);
        return;
      }
      const apps2 = this.appInMainViewPlugin.currentPageVisibleApps;
      if (apps2) {
        this._visibleApps = apps2;
      }
      set(this._visibleApps);
      return this._addAppInMainViewListener("appMenuChange", (apps3) => {
        this._visibleApps = new Set(
          [...apps3.entries()].filter(([_, { status }]) => status === "visible").map(([appId]) => appId)
        );
        set(this._visibleApps);
      });
    }));
...
async function createFastboard(_a) {
  ...
  var _a2, _b2;
  const isEnableAppliancePlugin = (enableAppliancePlugin == null ? void 0 : enableAppliancePlugin.cdn.fullWorkerUrl) && (enableAppliancePlugin == null ? void 0 : enableAppliancePlugin.cdn.subWorkerUrl) ? true : false;
  const joinRoomParamsWithPlugin = ensure_official_plugins(joinRoomParams);
  let _ApplianceMultiPlugin;
  ...
  if (enableAppInMainViewPlugin && _AppInMainViewPlugin) {
    appInMainViewPluginInstance = await _AppInMainViewPlugin.getInstance(
      manager,
      enableAppInMainViewPlugin === true ? void 0 : enableAppInMainViewPlugin
    );
  }
  ...
  if (room.isWritable && ((_a2 = room.floatBarOptions) == null ? void 0 : _a2.colors.length)) {
    const colors = (_b2 = room.floatBarOptions) == null ? void 0 : _b2.colors;
...
var FastboardPlayerBase = class {
  constructor(sdk, player, manager, syncedStore, appliancePlugin, appInMainViewPlugin) {
    this.sdk = sdk;
    this.player = player;
    this.manager = manager;
    this.syncedStore = syncedStore;
...
async function replayFastboard(_a) {
  ...
  const player = await sdk.replayRoom(
    __spreadProps(__spreadValues({}, replayRoomParamsWithPlugin), {
      useMultiViews: true
    }),
    callbacks
  );

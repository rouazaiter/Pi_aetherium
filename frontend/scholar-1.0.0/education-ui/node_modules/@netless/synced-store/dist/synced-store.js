...
const sideEffectManager = require("side-effect-manager");
const valueEnhancer = require("value-enhancer");
const whiteWebSdk = require("white-web-sdk");
...
class Refine {
  constructor(refState, defaultState) {
    ...
    __publicField(this, "refKeys", /* @__PURE__ */ new Set());
    __publicField(this, "genKey", () => {
      let key;
      do {
...
class Storage {
  constructor({
    ...
    __publicField(this, "_isWritable$");
    __publicField(this, "_refine");
    __publicField(this, "_sideEffect", new sideEffectManager.SideEffectManager());
    __publicField(this, "_events", new remitter.Remitter());
    /**
    ...
    const ensureInitState = (plugin) => {
      return valueEnhancer.subscribe(isWritable$, (isWritable) => {
        if (isWritable) {
    ...
    const listenNamespace = () => {
      let propsDisposer;
      const reactionDisposer = whiteWebSdk.reaction(
        getRawState,
        () => {
    ...
    this._sideEffect.addDisposer(
      valueEnhancer.subscribe(this._plugin$, (plugin) => {
        const disposers = [];
...
class SyncedStore {
  constructor(displayer, invisiblePlugin$, isRoomWritable$) {
    __publicField(this, "displayer");
    __publicField(this, "_plugin$");
    __publicField(this, "_isPluginWritable$");
    __publicField(this, "_isRoomWritable$");
    __publicField(this, "_sideEffect", new sideEffectManager.SideEffectManager());
    ...
    this._room = room;
    this._isPluginWritable$ = valueEnhancer.combine(
  ...
  addPluginWritableChangeListener(listener) {
    return valueEnhancer.reaction(this._isPluginWritable$, listener);
  }
  get isRoomWritable() {
  ...
  addRoomWritableChangeListener(listener) {
    return valueEnhancer.reaction(this._isRoomWritable$, listener);
  }
...
const _SyncedStorePlugin = class _SyncedStorePlugin extends whiteWebSdk.InvisiblePlugin {
  static async init(displayer) {
    const isRoomWritable$ = valueEnhancer.val(false);
    const updateRoomWritable = () => valueEnhancer.setValue(isRoomWritable$, whiteWebSdk.isRoom(displayer) && displayer.isWritable);
    updateRoomWritable();
    displayer.callbacks.on("onEnableWriteNowChanged", updateRoomWritable);
    const invisiblePlugin$ = valueEnhancer.val(
      displayer.getInvisiblePlugin(_SyncedStorePlugin.kind)
    );
    ...
    const createSyncedStore = async (isRoomWritable) => {
      if (isRoomWritable && whiteWebSdk.isRoom(displayer)) {
        try {
          ...
          valueEnhancer.setValue(invisiblePlugin$, plugin);
          valueEnhancer.unsubscribe(isRoomWritable$, createSyncedStore);
        } catch (e) {
    ...
    const removeWritableListener = (plugin) => {
      if (plugin) {
        valueEnhancer.unsubscribe(isRoomWritable$, createSyncedStore);
        valueEnhancer.unsubscribe(invisiblePlugin$, removeWritableListener);
      }
    };
    if (!invisiblePlugin$.value && whiteWebSdk.isRoom(displayer)) {
      valueEnhancer.subscribe(isRoomWritable$, createSyncedStore);
      valueEnhancer.subscribe(invisiblePlugin$, removeWritableListener);
    }
  ...
  static onCreate(plugin) {
    const invisiblePlugin$ = _SyncedStorePlugin.invisiblePlugins.get(
      plugin.displayer
    );
    invisiblePlugin$ && valueEnhancer.setValue(invisiblePlugin$, plugin);
  }
  constructor(context) {
    super(context);
    const invisiblePlugin$ = _SyncedStorePlugin.invisiblePlugins.get(
    ...
    invisiblePlugin$ && valueEnhancer.setValue(invisiblePlugin$, this);
  }
};
__publicField(_SyncedStorePlugin, "kind", "SyncedStore");
__publicField(_SyncedStorePlugin, "invisiblePlugins", /* @__PURE__ */ new Map());
...
//# sourceMappingURL=synced-store.js.map

...
export declare interface View {
    ...
    /**
     * 在 macOS 上，是否把 ``command`` 滚轮识别为缩放，默认为 ``false``。
     */
...
export declare type ViewCallbacks = {
    ...
    onActiveHotkey: (hotKey: string)=>void;
};
...
export declare interface Displayer<CALLBACKS extends DisplayerCallbacks = DisplayerCallbacks> {
    ...
    readonly useMultiViews: boolean;
    ...
    /**
     * 在 macOS 系统上，是否允许按住 ``command`` 键并滚动来放大缩小白板。
     * 此选项不影响 ``control`` 滚轮的放大缩小行为。
     * 默认为 ``false``。
     */
    commandWheelToZoom: boolean;
    ...
    /**
     * 读取场景背景图 ``scene.ppt.src`` 并生成他的 base64 URL 
     */
    generateScreenshot(scenePath?: string, width?: number, height?: number): Promise<string>;

    /**
     * 生成场景预览，和 {@link scenePreview} 的区别是此函数会选取一个框住所有元素的视角，而 scenePreview 选取的是用户当前视角 
     */
    fillSceneSnapshot(scenePath: string, div: HTMLElement, width?: number, height?: number, engine?: RenderEngine): void;

    /**
     * 生成屏幕快照，并写入指定的 CanvasRenderingContext2D 对象中。
     * 调用此方法时可能存在一些图片还未加载完成从而不会渲染出来。
     * 
     * @param context canvas 2D 上下文, 你可以通过 ``canvas.getContext("2d")`` 来获取
     * @param scenePath 要截图的场景路径
     * @param width canvas 的宽度，单位像素
     * @param height canvas 的高度，单位像素
     * @param camera 目标视角
     * @param ratio 缩放比例，默认为 1
     * 
     * @example
     * ```javascript
     * const canvas = document.createElement("canvas");
     * const { scenePath } = room.state.sceneState;
     * const { width, height, ...camera } = room.state.cameraState;
     * canvas.width = width;
     * canvas.height = height;
     * room.screenshotToCanvas(canvas.getContext("2d"), scenePath, width, height, camera);
     * ```
     */
    screenshotToCanvas(context: CanvasRenderingContext2D, scenePath: string, width: number, height: number, camera: Camera, ratio?: number): void;

    /**
     * **实验性** 等待目标场景的图片加载完成，然后生成屏幕快照，写入指定的渲染上下文中。
     * 
...
export declare type DisplayerCallbacks = {
     ...
     */
    onEnableWriteNowChanged: (isReadOnly: boolean)=>void;
    /**
...
export declare interface Room extends Displayer {
    ...
    /**
     * 是否使用原生剪贴板。
     * 设为 ``true`` 后，快捷键里的复制和粘贴将失效而使用原生 ``copy`` ``paste`` 事件。
     * 此功能需要浏览器支持以及用户授予剪贴板权限，如果失败，将自动回退到虚拟剪贴板实现。
     */
    ...
    /**
     * 导出特定场景路径的板书内容（包含 ppt 内容），可以直接保存为二进制文件(废弃)。
     * @param scenePath 需要导出场景的路径，必须是 ``ScenePathType`` 为 ``page`` 的路径。
     */
     ...
     * @param overrideName 不使用文件内的 name，改为覆盖指定场景
     * @returns 板书内部的内容，一定会存在 name 字段，该字段不可更改；最后插入的板书路径为 dir + name。如果想修改板书路径，可以再调用 mvScene API 进行修改。
     */
    importScene(dir: string, payload: Blob, overrideName?: string): Promise<SceneDefinition>;
...
export declare type RoomCallbacks = DisplayerCallbacks & {
    /**
     * 已经建立 WebSocket 连接，将要进入初始化阶段。
     * 
     * @param duration 建立连接的时间，单位是毫秒
     */
    ...
    /**
     * 当前场景里的元素数量触发了限制，当 ``soft`` 为 ``true`` 时触发软限制（此时仍可插入元素），否则无法再添加元素且房间状态会被强制回退。 
     */
...
export declare type ScenesCallbacksNode = Callbacks<ScenesCallbacks> & {
    /**
     * 监听的场景组地址 
     */
    path: string;
...
export declare type MemberState = {
     ...
     * 是否自动选中粘贴产生的图形 
     */
    autoSelectPaste?: boolean;
    /**
     * 是否自动选中拷贝（浮动工具栏上的按钮）产生的图形 
     */
    autoSelectDuplicate?: boolean;
    /**
     * text 打完字后自动切到选择工具 
     */
    textCompleteToSelector?: boolean;
    /**
     * 填充色，只有圆、矩形等封闭图形可以有填充色 
     */
    fillColor?: Color;
    /**
     * 覆盖 text 默认字体大小，如果不设置则默认使用 textSize 即上一次设置的文字大小 
     */

...
export class FileService {
  ...
  // Récupère ou génère un Quizz IA pour une session
  getQuiz(fileId: number): Observable<any[]> {
    return this.http.get<any[]>(`${this.api}/quiz/${fileId}`);
  }

  // Interroge TOUT le Drive (tous les bilans fusionnés)
  globalChat(userId: number, question: string): Observable<any> {
    return this.http.post(`${this.api}/global-chat`, { userId, question });
  }

  saveQuizResult(result: any): Observable<any> {
    return this.http.post(`${this.api}/quiz-results`, result);
  }

  getUserStats(userId: number): Observable<any> {
    return this.http.get(`${this.api}/stats/${userId}`);
  }

  analyzePerformance(userId: number): Observable<any> {
    return this.http.get(`${this.api}/stats/analysis/${userId}`);
  }

...
export class DriveComponent implements OnInit {
  ...
  downloadPDF() {
    ...
    // On récupère le quizz aussi s'il existe pour l'inclure
    this.fileService.getQuiz(this.selectedVideo.id).subscribe({
      next: (quiz) => {
        this.pdfService.generateStudyGuide(
          this.selectedVideo.name,
          this.summaryLines,
          this.summaryMeta.keywords,
          this.summaryMeta.usage,
          quiz
        );
      },
      error: () => {
        // Si pas de quizz, on génère juste le résumé
        this.pdfService.generateStudyGuide(
          this.selectedVideo.name,
          this.summaryLines,
          this.summaryMeta.keywords,

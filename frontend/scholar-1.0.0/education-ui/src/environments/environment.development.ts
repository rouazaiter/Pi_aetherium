export const environment = {
   ...
   * et `proxy.conf.json` redirige vers Spring (http://localhost:8081). Évite les 404 si le navigateur
   * n’atteint pas directement le port 8081.
   *
   * Si vous servez le front autrement (sans proxy), mettez l’URL du backend, ex. :
   * `http://localhost:8081` — doit être identique à `server.port` dans `application.properties`.
   */
  apiUrl: '',
  /**
